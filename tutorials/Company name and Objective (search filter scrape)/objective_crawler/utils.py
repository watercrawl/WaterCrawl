...
def _debug_print(message: str, debug_mode: bool = False) -> None:
    """Print debug message if debug mode is enabled."""
    if debug_mode:
        print(f"\033[35m[DEBUG] {message}\033[0m")
...
def _debug_print_content(content: str, title: str, debug_mode: bool = False, max_chars: int = 1000) -> None:
    """Print content with title for debugging."""
    if debug_mode:
        print(f"\033[35m===== {title} =====\033[0m")
        if len(content) > max_chars:
            print(f"\033[35m[Content truncated to {max_chars} chars]\033[0m")
            content = content[:max_chars] + "..."
        print(f"\033[35m{content}\033[0m")
        print(f"\033[35m===== END {title} =====\033[0m")
