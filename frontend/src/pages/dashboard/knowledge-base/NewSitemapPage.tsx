...
const NewSitemapPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setItems([
      ...
      {
        label: t('knowledgeBase.import.newSitemap'),
        href: `/dashboard/knowledge-base/${knowledgeBaseId}/import/new-sitemap`,
        current: true,
      },
    ]);
  ...
  const handleSitemapEvent = (event: SitemapEvent) => {
    if (event.type === EventType.State) {
      ...
      if (request.status === SitemapStatus.Finished) {
        navigate(
          `/dashboard/knowledge-base/${knowledgeBaseId}/import/select-sitemap/${request.uuid}`
        );
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-foreground">{t('knowledgeBase.import.newSitemapDesc')}</p>
      </div>
      ...
      <div className="rounded-lg bg-card p-6 shadow">
        {activeSitemap ? (
                ...
                <div className="mb-2 flex items-center justify-between">
                  <div>
                    <span className="inline-block text-xs font-semibold text-primary">
                      ...
                      <StatusBadge status={activeSitemap.status as SitemapStatus} />
                    </span>
                  </div>
                </div>
                <div className="flex h-10 overflow-hidden rounded bg-primary/20 text-xs">
          ...
          <div className="space-y-4">
            ...
            <SitemapForm
              onSitemapEvent={handleSitemapEvent}
              hideApiDocs={true}
              hideResultsTab={true}
            />

            {/* Add instructions for the user */}
            <div className="mt-6 rounded-lg border border-alert-info-border bg-alert-info-bg p-4">
