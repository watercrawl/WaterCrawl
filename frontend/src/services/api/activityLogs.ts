...
export const activityLogsApi = {
  ...
  ): Promise<PaginatedResponse<CrawlRequest>> {
    return api
      .get<PaginatedResponse<CrawlRequest>>(`/api/v1/core/crawl-requests/`, {
  ...
  async getCrawlRequest(requestId: string): Promise<CrawlRequest> {
    return api
      .get<CrawlRequest>(`/api/v1/core/crawl-requests/${requestId}/`)
      .then(({ data }) => data);
  },
  ...
  async getCrawlResults(
    ...
    page: number = 1,
    pageSize: number = 25,
    prefetched: boolean = true
  ): Promise<PaginatedResponse<CrawlResult>> {
    return api
      .get<
        PaginatedResponse<CrawlResult>
      >(`/api/v1/core/crawl-requests/${requestId}/results/`, { params: { page, page_size: pageSize, prefetched } })
      .then(({ data }) => data);
  },
