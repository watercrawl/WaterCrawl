...
export const sitemapApi = {
  ...
  async get(id: string) {
    return api.get<SitemapRequest>(`/api/v1/core/sitemaps/${id}/`, {
      params: {
        prefetched: true
      }
    }).then(({ data }) => data);
  ...
  async list(page = 1, status?: string) {
    return api.get<PaginatedResponse<SitemapRequest>>('/api/v1/core/sitemaps/', {
      params: {
        page,
        page_size: 10,
        status
  ...
  async subscribeToStatus(uuid: string, onEvent: (data: SitemapEvent) => void, onEnd?: () => void) {
    return api.subscribeToSSE<SitemapEvent>(`/api/v1/core/sitemaps/${uuid}/status/`, {
      params: {
        prefetched: true
      }
    }, onEvent, onEnd);
