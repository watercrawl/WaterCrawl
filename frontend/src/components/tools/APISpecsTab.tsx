...
import { PlusIcon, TrashIcon, ChevronDownIcon, ChevronRightIcon, DocumentTextIcon, InformationCircleIcon, ShieldCheckIcon, ArrowUpTrayIcon, CommandLineIcon, PencilIcon } from '@heroicons/react/24/outline';
...
const APISpecsTab: React.FC = () => {
  ...
  const handleEditClick = (spec: APISpec) => {
    setEditingSpec(spec);
    setSpecName(spec.name);
    setBaseUrl(spec.base_url || '');
    // setSpecJson(JSON.stringify(spec.api_spec, null, 2));
    setSpecParameters(spec.parameters.map(p => ({
      tool_parameter_type: p.tool_parameter_type,
      name: p.name,
      value: '' // Masked values from backend
    })));
    setSpecHasBaseUrl(!!spec.base_url);
  ...
  const handleCreateApiSpec = async () => {
    if (!specName.trim() || (!editingSpec && !specJson.trim())) {
    ...
    const invalidParams = specParameters.some(p => !p.name.trim() || (!editingSpec && !p.value.trim()));
    if (invalidParams) {
    ...
    try {
      ...
      if (editingSpec) {
        await toolsApi.updateApiSpec(editingSpec.uuid, request);
        toast.success(t('tools.apiSpec.updateSuccess'));
      } else {
  ...
  return (
    <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-x-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">{spec.base_url}</p>
      ...
      <Modal
        ...
        title={editingSpec ? t('tools.apiSpec.editTitle') : t('tools.apiSpec.createTitle')}
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={creating}
            >
              {creating ? t('common.saving') : (editingSpec ? t('common.save') : t('common.create'))}
            </Button>
          </>
        }
      >
        <div className="space-y-4">
          ...
          {!editingSpec && <div>
            ...
            <textarea
              ...
              rows={8}
              className="block w-full rounded-lg border border-input-border bg-input px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary font-mono disabled:opacity-50 disabled:cursor-not-allowed ltr"
          ...
          <div>
            ...
            <input
              ...
              className={`block w-full rounded-lg border bg-input px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary ltr ${
          ...
          <div className="rounded-lg bg-info-soft/50 border border-info/20 p-3">
            <div className="flex gap-2">
              ...
              <div className="text-xs text-foreground space-y-1">
                ...
                <code className="block mt-1 px-2 py-1 bg-muted rounded text-xs font-mono ltr">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`block w-full rounded-lg border px-2 py-1.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary bg-input ${
                            isRequired && (!param.value || !param.value.trim()) && !editingSpec
                              ? 'border-warning'
                              : 'border-input-border'
                          }`}
