...
const Feed: React.FC<FeedProps> = ({
    ...
    const renderMessage = (message: FeedMessage) => (
        <div
            key={message.id}
            className={`flex items-start py-1 text-xs border-l-2 px-2 rounded ${message.type === 'error'
        ...
        <div className={`bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 ${className}`}>
            ...
            <div className="px-3 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <h3 className="text-xs font-medium text-gray-700 dark:text-gray-300 flex items-center">
                    <span>{title}</span>
                    {loading && (
                        <div className="ml-2 animate-spin inline-block w-3 h-3 border-2 border-gray-300 border-t-primary-500 rounded-full" />
                    )}
                    {messages.length > 0 && (
                        <span className="ml-2 text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-full">
                            {messages.length}
                        </span>
                    )}
                </h3>
                ...
                >
                    {expanded ? 'Show Latest' : 'Show Trace'}
                </button>
            </div>
