...
class Plan(BaseModel):
    ...
    group = models.CharField(
        _('Group'),
        max_length=100,
        choices=consts.PLAN_GROUP_CHOICES
    )
    ...
    daily_page_credit = models.IntegerField(
        _('Daily page credit'),
        help_text=_('Number of pages that can be crawled per day. resets at midnight. -1 for unlimited'),
        default=100
    )
    crawl_max_depth = models.IntegerField(
        _('Crawl max depth'),
        help_text=_('Max depth of crawling.'),
        default=3
    )
    crawl_max_limit = models.IntegerField(
        _('Crawl max limit'),
        help_text=_('Max number of pages to crawl.'),
        default=100
    )
...
class Subscription(BaseModel):
    ...
    remain_page_credit = models.IntegerField(
        _('Remain page credit'),
        default=0
    )
    remain_daily_page_credit = models.IntegerField(
        _('Remain daily page credit'),
        default=0
    )
    ...
    current_period_start_at = models.DateTimeField(
        _('Current period start datetime'),
        null=True,
        blank=True
    )
    current_period_end_at = models.DateTimeField(
        _('Current period end datetime'),
        null=True,
        blank=True
    )
