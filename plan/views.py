...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        ...
        description=_('Returns detailed information about a specific subscription plan.'),
        tags=['Subscriptions']
    )
)
...
@extend_schema_view(
    current=extend_schema(
        ...
        description=_('Returns detailed information about the team\'s current subscription.'),
        tags=['Subscriptions'],
        request=None,
        responses={
            ...
            200: OpenApiResponse(
                response={
                    ...
                    "properties": {
                        "redirect_url": {
                            "type": "string",
                            "description": _("The URL to redirect the user to for payment processing.")
                        }
                    }
                },
                description=_('Returns the URL to redirect the user to for payment processing.')
            )
        }
    ),
...
class SubscriptionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def current(self, request, pk=None):
        ...
        return Response(
            serializers.TeamPlanSerializer(service).data
        )
    ...
    def start(self, request):
        ...
        if not plan.is_default:
            url = StripeService().start_payment(
                plan,
    ...
    def cancel(self, request):
        ...
        return Response(
            data={
                "redirect_url": service.cancel_subscription(request.current_team)
            }
        )
...
@extend_schema_view(
    post=extend_schema(
        ...
        description=_('Processes webhook events from Stripe for subscription and payment updates.'),
        tags=['Webhooks'],
        responses={204: None}
    )
)
